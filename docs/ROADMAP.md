...
The next implementation target is Phase A only: add the protocol message model, synthetic fixtures, parser tests, and a protocol summary report.

Do not add a live listener, external bind, execution step, or live route mutation until the protocol, session, share intake, accounting, plan, dry-run, and readiness reports are all stable.
