...
try {
    ...
    $CorruptLedger = "crates\dbyte-agent\fixtures\corrupt-ledger.events"
    ...
    Write-Host "== corrupt ledger check =="
    cargo run --manifest-path $Manifest -- --config $Config --ledger $CorruptLedger check-ledger
    if ($LASTEXITCODE -ne 1) {
        throw "corrupt ledger check should fail with exit code 1, got $LASTEXITCODE"
    }
